...
class VoiceClient:
    ...
    async def _try_connection(self, guild_id: hikari.Snowflake) -> None:
        ...
        if not pending_connection.endpoint or not pending_connection.session_id or not pending_connection.token:
            return
    ...
    async def _server_update(self, event: hikari.VoiceServerUpdateEvent) -> None:
        ...
        pending_connection: PendingConnection = self._pending_connections[event.guild_id]
